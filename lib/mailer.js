...
function isTransientNetworkError(err) {
  ...
  if (code === 'ETIMEDOUT' || code === 'ESOCKETTIMEDOUT' || code === 'ECONNRESET' || code === 'EHOSTUNREACH') {
    return true;
  }
  if (msg.includes('connection timeout') || msg.includes('greeting never received') || msg.includes('socket timed out') || msg.includes('connection closed')) {
    return true;
  }
  return false;
}
...
function createTransport(config) {
  return nodemailer.createTransport({
    ...
    connectionTimeout: 25000, // 建立连接超时 25 秒
    greetingTimeout: 25000,   // 等待问候语超时 25 秒
    socketTimeout: 45000,     // 传输数据超时 45 秒
  });
}
    ...
    try {
      ...
      while (tryCount < maxTries) {
        ...
        try {
          ...
          if (tryCount < maxTries && isTransientNetworkError(sendErr) && !abortFlag) {
            console.warn(`[mailer] 部门 [${dept}] 邮件发送遭遇网络瞬时异常 (${sendErr.code || sendErr.message})，1.5 秒后自动重试...`);
...
module.exports = { getProgressEmitter, sendBatch, createTransport, abortSend, isTransientNetworkError };
